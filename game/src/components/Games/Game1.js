...
function Game1() {
  ...
  const handleDrop = (e, target) => {
    ...
    const currentCorrectAnswer = questions[difficulty][shuffledQuestionIndices[currentQuestion]]
      ?.correctAnswer;
    ...
    <button className="theme-button" onClick={toggleTheme}> Dark Mode </button>
      ...
      <div
        className={`target ${
          questionAnswered ||
          draggedItem !==
            questions[difficulty][shuffledQuestionIndices[currentQuestion]]?.correctAnswer
            ? 'disabled'
            : ''
        }`}
      ...
      <button className="submit-button" onClick={handleNext}>
          {currentQuestion === shuffledQuestionIndices.length - 1 ? 'Submit' : 'Next'}
        </button>
       {/* Reset Button */}
